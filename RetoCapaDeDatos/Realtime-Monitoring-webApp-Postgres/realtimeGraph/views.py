...
def get_new_query(request, **kwargs):
    data_result = {}
    ...
    for location in locations:
        ...
        avgVal = locationData.aggregate(
            Avg('value'))['value__avg']
        countVal = locationData.aggregate(
            Count('value'))['value__count']
        varianceVal = locationData.aggregate(
            Variance('value'))['value__variance']
        datas=[v.value for v in locationData.all()]
        d= locationData.first()
        np_array = np.array(datas)
        c = locationData.all()
        ...
        data.append({
            ...
            'max': maxVal if maxVal != None else 0,
            'avg': round(avgVal if avgVal != None else 0, 2),
            'count': countVal if countVal != None else 0,
            'variance': round(varianceVal if varianceVal != None else 0,2),
            'percentile_30': round(np.percentile(np_array, 30),2),
            'percentile_50': round(np.percentile(np_array, 50),2),
            'percentile_80': round(np.percentile(np_array, 80),2),
            'percentile_90': round(np.percentile(np_array, 90),2),
            'percentile_95': round(np.percentile(np_array, 95),2),
            'percentile_99': round(np.percentile(np_array, 99),2),
            'percentile_99.5': round(np.percentile(np_array, 99.5),2),
        })
